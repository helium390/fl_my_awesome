import 'package:flutter/material.dart';

class AppColor {
  static const primaryColor = Color(0xFFFFFFFF);
  static const primaryColorDark = Color(0xFFCCCCCC);
  static const accentColor = Color(0xFFC8232B);

  static const bgWhite = Color(0xFFF9F9F9);

  static const white = Colors.white;
  static const black = Colors.black;

  static const grey1 = Color(0xFF666666);

  static const grBorder = Color(0xFFCBCBCB);

  static const black1 = Color(0xFF2B2B2B);
  static const black2 = Color(0xFF776F6F);
  static const black3 = Color(0xFF8C8C8C);
  static const black6 = Color(0xFFD2D2D2);

  static const red1 = Color(0xFFC20E11);
  static const red2 = Color(0xFFF7D4D4);

  static const yellow1 = Color(0xFFF4D44E);

  static const hint1 = Color(0xFFD0D0D0);

  static const green1 = Color(0xFF1A9317);

  static const blue1 = Color(0xFF007AFF);
  static const blue2 = Color(0xFF0B1CB2);
  static const blue3 = Color(0xFF005DCE);
  static const blue4 = Color(0xFFC1DEFF);
  static const blue5 = Color(0xFF0678F1);
  static const blue8 = Color(0xFF281E39);
  static const blue9 = Color(0xFF0330C0);

  static const greyBg = Color(0xFFF5F5F5);
  static const mainColor = Color(0xFF222222);
  static const accentBorder = Color(0x1A222222);

  static const textColor = Color(0xFF262626);
  static const hintColor = Color(0xFFB5B7BB);
  static const Color buttonText = Color(0xFF333333);
  static const Color errorText = Color(0xFFE75973);
  static const Color noErrorText = Color(0xFF8B8B8D);
  static const Color toastColor = Color(0xFF303030);
}
